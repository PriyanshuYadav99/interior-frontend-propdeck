import { API_BASE_URL as BACKEND_URL } from "../config/env";
...
export const pauseToolTimer = (toolName) => {
  ...
  const elapsed = Math.floor(
    (Date.now() - toolTimers[toolName].startTime) / 1000,
  );
  toolTimers[toolName].totalSeconds += elapsed;
  toolTimers[toolName].startTime = null;
  console.log(
    `[Tracker] ⏸ Timer paused — ${toolName} (+${elapsed}s, total=${toolTimers[toolName].totalSeconds}s)`,
  );
};
...
export const logVirtualTourSelection = async (
  category,
  placeName,
  placeId,
  photoUrl,
...
// ─── LifeEcho selection ──────────────────────────────────────
export const logLifeEchoSelection = async ({
  isCustom,
  scenarioId = null,
  scenarioTitle = "",
  scenarioIcon = "clock",
