...
export const generateDesignAsync = async (
  roomType,
  style,
  customPrompt,
  clientName = "skyline",
...
export const incrementGeneration = async (
  sessionId,
  roomType,
  style,
  customPrompt,
 ...
 */
export const searchVirtualTour = async (
  location,
  category = "dining",
  radius = 5000,
) => {
  console.log("[API] searchVirtualTour called with:", {
    location,
    category,
    radius,
  });
 ...
 */
export const getVirtualTourDirections = async (
  origin,
  destination,
  mode = "driving",
) => {
  console.log("[API] getVirtualTourDirections called with:", {
    origin,
    destination,
    mode,
  });
...
export const getPlaceDetails = async (placeId) => {
  ...
  const response = await fetch(
    `${API_BASE_URL}/api/virtual-tour/place-details/${placeId}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
