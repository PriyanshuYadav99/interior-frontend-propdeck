...
const VirtualTour = ({ onBack, isEmbedded = false, initialPlace = null, initialMode = 'map', initialCategory = 'dining' }) => {
  ...
  const initializeMap = () => {
    ...
    try {
      ...
      const originMarker = new window.google.maps.Marker({
        position: { lat: origin.lat, lng: origin.lng }, map, title: origin.name,
        icon: { path: window.google.maps.SymbolPath.CIRCLE, scale: 10, fillColor: '#10b981', fillOpacity: 1, strokeColor: 'white', strokeWeight: 3 }
      });
      markersRef.current.push(originMarker);
      places.forEach((place, index) => {
        const marker = new window.google.maps.Marker({
          position: { lat: place.coordinates.lat, lng: place.coordinates.lng }, map, title: place.name,
          label: isCustomSearch ? undefined : { text: `${index + 1}`, color: 'white', fontSize: '12px', fontWeight: 'bold' },
          icon: { path: window.google.maps.SymbolPath.CIRCLE, scale: isCustomSearch ? 18 : 15, fillColor: isCustomSearch ? '#ef4444' : '#3b82f6', fillOpacity: 1, strokeColor: 'white', strokeWeight: 2 }
        });
  ...
  const displayDirections = () => {
    ...
    const renderer = new window.google.maps.DirectionsRenderer({
      ...
      polylineOptions: { strokeColor: '#9333ea', strokeWeight: 4 }
    });
  ...
  const searchNearbyPlaces = async () => {
    if (!searchLocation.trim()) { setError('Please enter a location or keyword'); return; }
    setLoading(true); setError(''); setSelectedPlace(null); setDirections(null); setIsCustomSearch(true); setShowStreetView(false);
    try {
      // Detect if user typed a keyword (e.g. "Indian restaurant") vs an address/coords
      const isLatLng = /^-?\d+\.?\d*,\s*-?\d+\.?\d*$/.test(searchLocation.trim());
      const looksLikeAddress = isLatLng || searchLocation.includes(',');
      const isKeyword = !looksLikeAddress;
      ...
      if (isKeyword) {
        ...
        result = await searchVirtualTour(searchLocation, selectedCategory, SEARCH_RADIUS, true);
      }

      if (!result.success) throw new Error(result.error || 'Search failed');
      if (!result.places || result.places.length === 0) {
        setError(`No results found for "${searchLocation}".`);
        setShowMap(false);
      } else {
  ...
  const handlePlaceClick = async (place) => {
    ...
    // ✅ TRACKING: only log if place has a real ID
    if (place?.id && place?.name) {
      logVirtualTourSelection(
        selectedCategory,
        place.name,
        place.id,
    ...
    if (origin) {
      try {
        ...
        setDirections({ distance: { text: `${place.distance} km` }, duration: { text: `${Math.floor(place.distance * 3)} mins` } });
      }
    }
  };
...
<div style={{ width: '100%', height: '100%', background: 'transparent', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      {/* HEADER */}
<div style={{ padding: '0 1.5rem', background: 'transparent', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>        <h1 style={{ fontSize: '1.4rem', fontWeight: '700', margin: 0, color: '#1f2937' }}>Virtual Tour</h1>
      ...
      {/* CATEGORIES + SEARCH */}
<div style={{ padding: '0.5rem 1.5rem 0.75rem', background: 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', flexShrink: 0 }}>        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
          {categories.map(cat => (
            <button key={cat.id} onClick={() => handleCategoryChange(cat.id)} disabled={loading}
              style={{ padding: '0.45rem 0.9rem', borderRadius: '20px', border: '1.5px solid #d1d5db', background: selectedCategory === cat.id ? '#3b82f6' : 'white', color: selectedCategory === cat.id ? 'white' : '#374151', cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.85rem', fontWeight: '500', opacity: loading ? 0.6 : 1 }}>
        ...
        <div style={{ position: 'relative', width: '280px', flexShrink: 0 }}>
          ...
          <input type="text" placeholder="Search place or keyword (e.g. Indian restaurant)" value={searchLocation}
      ...
      {/* MAIN CONTENT */}
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', background: 'transparent', gap: '0.75rem', padding: '0 1rem 1rem' }}>
        {(showMap || showStreetView) && places.length > 0 ? (
          <>
            {/* Places List */}
            <div style={{ width: '45%', background: '#F5F7FA', borderRadius: '12px', overflowY: 'auto', padding: '0.75rem', flexShrink: 0 }}>
              ...
              {places.map((place) => (
                <div key={place.id} onClick={() => handlePlaceClick(place)}
                  style={{ background: selectedPlace?.id === place.id ? '#f0fdf4' : '#F5F7FA', border: selectedPlace?.id === place.id ? '2px solid #10b981' : '1px solid #e5e7eb', borderRadius: '10px', padding: '0.75rem', marginBottom: '0.6rem', cursor: 'pointer', display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
  ...
  {/* Distance pill - outlined style like Figma */}
  <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', padding: '0.25rem 0.7rem', background: 'white', border: '1.5px solid #d1d5db', borderRadius: '20px', fontSize: '0.75rem', color: '#374151', fontWeight: '600' }}>
  ...
  {place.rating && (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', padding: '0.25rem 0.7rem', background: '#FBBF24', borderRadius: '20px', fontSize: '0.75rem', color: 'white', fontWeight: '600' }}>
      <Star size={12} fill="white" color="white" strokeWidth={0} />
