...
const VirtualTour = ({
  ...
  useEffect(() => {
    if (mapLoaded) autoLoadApartment();
  }, [mapLoaded]);
  ...
  const autoLoadApartment = async () => {
    ...
    try {
      ...
      if (loadedPlaces.length === 0) {
        setError("No places found within 5km.");
        setShowMap(true);
      } else {
    ...
    try {
      const centerCoords =
        initialPlace && !initialHandled.current === false
          ? {
              lat: initialPlace.coordinates.lat,
              lng: initialPlace.coordinates.lng,
            }
  ...
  const getCategoryIcon = (iconName) => {
    ...
    return I ? <I size={16} strokeWidth={2} /> : null;
  };
    ...
    >
      ...
      >
        ...
        <div style={{ position: "relative", width: "280px", flexShrink: 0 }}>
          ...
          <input
            type="text"
            placeholder="Search place or keyword (e.g. Indian restaurant)"
            ...
            >
              ...
              {places.map((place) => (
                <div
                  ...
                  style={{
                    background:
                      selectedPlace?.id === place.id ? "#F8F5EF" : "#F3EEE5",
                    border:
